...
def fibonacci(num):
    number=np.zeros(binary_dim)
    for i in range(binary_dim):

          number[i]=foundFib(num+i)
    return number/2000
...
def foundFib(num):
    if (num==0):
        ...
        return foundFib(num-1)+foundFib(num-2)
...
for i in range(100000):
    ...
    layer_2_deltas = list()
    layer_1_values = list()
    layer_1_values.append(sigmoid(np.dot(num[0], synapse_0)))

    result=np.zeros(6)

    X= np.array([[num[1]]])
    for position in range(6):
        Y = np.array([[num[position+2]]]).T
